...
def load_cdss_data(filename):
    ...
    if "shear_strain" not in cols:
        raise KeyError(f"'shear_strain' column not found in {filename}")

    if "s_xz" not in cols:
        raise KeyError(f"'s_xz' column not found in {filename}")
...
def extract_params(fname):
    ...
    Extract:
        RF1.5_Fric0.2_yMod30E9
    """

    match = re.search(
        r'_([0-9\.E\+\-]+)_([0-9\.E\+\-]+)_([0-9\.E\+\-]+)$',
        fname
    )
...
def process_file(filename):
    try:
        ...
        # Internal stress: s_xz
        gsecs, ds = calculate_cycles(gamma, cols["s_xz"])

        # Optional top-cap stress: topcap_SXZ
        if "topcap_SXZ" in cols:
            gsecs_T, ds_T = calculate_cycles(gamma, cols["topcap_SXZ"])
        else:
            print(f"[INFO] topcap_SXZ column not found in {filename}; skipping top-cap calculation.")
            gsecs_T = None
            ds_T = None
        ...
        return [np.nan] * 4, [np.nan] * 4, None, None
...
def main():
    ...
    if has_topcap:
        G_T = np.array([
            r[3] if r[3] is not None else [np.nan] * 4
            for r in results
        ])

        D_T = np.array([
            r[4] if r[4] is not None else [np.nan] * 4
            for r in results
        ]) * 100.0
