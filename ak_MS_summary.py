...
def summarize_file(filename):
    try:
        ...
        bounds = bounds[:4]  # Use only first 4 cycles
...
def main():
    files = sorted(
        [f for f in os.listdir('.') if f.startswith("cyclic_MS_stress_NO") and f.endswith(".txt")],
        key=extract_number
    )
    results = []
    for f in files:
    ...
    with open("Gsec_D_Summary_MS.txt", "w") as out:
        out.write("File\tGsec_1\tGsec_2\tGsec_3\tGsec_4\tD_1\tD_2\tD_3\tD_4\n")
        for row in results:
            out.write(row[0] + "\t" + "\t".join(
                f"{x:.3e}" if not np.isnan(x) else "NaN" for x in row[1:]
            ) + "\n")

    print("[DONE] Results saved to Gsec_D_Summary_MS.txt")
